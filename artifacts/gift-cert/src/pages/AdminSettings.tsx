...
export default function AdminSettings() {
  ...
  const [cleanResult, setCleanResult] = useState<{ cleaned: number; ran: Date } | null>(null);

  const cutoffDate = formatKo(new Date(Date.now() - 6 * 30 * 24 * 60 * 60 * 1000));
  ...
  async function handleCleanup() {
    ...
    try {
      ...
      showToast(data.cleaned > 0 ? `✅ ${data.cleaned}건 개인정보 삭제 완료` : "✅ 삭제 대상이 없습니다", true);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 pb-16">
        ...
        {/* 개인정보 자동삭제 */}
        <div className="mt-4 bg-white rounded-2xl border border-slate-100 shadow-sm px-4 py-4 space-y-3">
          <div className="flex items-center gap-2">
            <span className="text-[16px]">🔒</span>
            <p className="text-[12px] font-black text-slate-500 uppercase tracking-wide">개인정보 자동삭제 정책</p>
          </div>

          <div className="bg-slate-50 rounded-xl px-3 py-3 space-y-1.5 text-[12px] text-slate-500">
            <p className="flex items-start gap-2"><span className="text-slate-300 flex-shrink-0 mt-0.5">•</span>고객 개인정보는 예약 접수일로부터 <span className="font-bold text-slate-700">6개월 후 자동 삭제</span>됩니다.</p>
            <p className="flex items-start gap-2"><span className="text-slate-300 flex-shrink-0 mt-0.5">•</span>삭제 항목: 성명, 연락처, 거래장소, 계좌정보, 채팅 내역</p>
            <p className="flex items-start gap-2"><span className="text-slate-300 flex-shrink-0 mt-0.5">•</span>매일 자동 실행 (서버 기준)</p>
            <p className="flex items-start gap-2"><span className="text-slate-300 flex-shrink-0 mt-0.5">•</span>현재 기준일: <span className="font-bold text-slate-700">{cutoffDate} 이전</span></p>
          </div>

          {cleanResult && (
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5 flex items-center justify-between">
              <p className="text-[12px] font-bold text-emerald-700">
                {cleanResult.cleaned > 0
                  ? `${cleanResult.cleaned}건 개인정보 삭제 완료`
                  : "삭제 대상 없음 (최신 상태)"}
              </p>
              <p className="text-[10px] text-emerald-400">
                {cleanResult.ran.toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}
              </p>
            </div>
          )}
          ...
          <button
            ...
            disabled={cleaning}
            type="button"
            className="w-full py-3 rounded-xl bg-slate-100 text-slate-600 text-[13px] font-bold hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 border border-transparent transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {cleaning ? (
              <span className="flex items-center justify-center gap-2">
