...
export default function AdminRevenue() {
  ...
  const buildGroups = useCallback((): DayGroup[] => {
    const filtered = allData.filter(
      (r) => r.status === "completed" && r.date && r.date >= startDate && r.date <= endDate
    );
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-100 sticky top-0 z-40 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-3.5 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-[16px] font-bold text-slate-800">매출 조회</h1>
            <p className="text-[11px] text-slate-400 mt-0.5">처리완료 기준</p>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 pb-16 space-y-3">
        ...
        {loading ? (
          ...
          <>
            {/* 합계 카드 */}
            <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-4 py-4 flex items-center justify-between">
              <div>
                <p className="text-[12px] font-bold text-indigo-700">기간 총 매입금액</p>
                <p className="text-[11px] text-indigo-400 mt-0.5">
                  {startDate === endDate ? formatDateKo(startDate) : `${formatDateKo(startDate)} ~ ${formatDateKo(endDate)}`}
                </p>
                <p className="text-[11px] text-indigo-400">{totalCount}건 처리완료</p>
              </div>
              <p className="text-[24px] font-black text-indigo-600 tracking-tight">{formatKRW(totalRevenue)}</p>
            </div>
                  ...
                  return (
                    <div key={g.date} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <p className="text-[16px] font-black text-emerald-600 tabular-nums">{formatKRW(g.total)}</p>
                          <span
                        ...
                        <div className="border-t border-slate-50">
                          ...
                          {g.rows.map((r, idx) => (
                            ...
                            >
                              ...
                              <p className="text-[13px] font-black text-emerald-600 tabular-nums whitespace-nowrap">{formatKRW(r.totalPayment)}</p>
                            </div>
                          ))}
                        </div>
                      )}
