        ...
        {numbers.map((num, idx) => (
          <div key={idx} className="flex items-center gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl border-2 border-rose-300 bg-white text-[14px] font-mono tracking-wider outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-100 transition-all placeholder:text-rose-300 text-rose-800"
            />
            {idx === 0 ? (
              <button
                type="button"
...
export default function MobileSelect() {
  ...
  function validate() {
    ...
    // 롯데모바일 23으로 시작하는 교환권: 번호 또는 이미지 중 하나 이상 필수
    ...
    if (hasLotte23) {
      ...
      if (!hasAnyLotteNum && lotte23UploadedCount === 0) {
        errs.lotte23 = "1개 이상의 교환권 번호 또는 이미지가 등록되어야 합니다.";
      }
    }
